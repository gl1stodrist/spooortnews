...
const Index = () => {
  ...
  // Ссылки на твои баннеры из облака
  const BANNER_1 = "https://i.ibb.co/hR0L7v6X/753-155.png";
  const BANNER_2 = "https://i.ibb.co/6R8mS96m/753-155-1.png";

  return (
    <div className="min-h-screen bg-[#0d0f12] text-white font-sans selection:bg-red-600">
      {/* ВЕРХНИЙ СТИКИ-БАННЕР */}
      ...
      <header className="border-b border-white/5 bg-[#161920]/90 backdrop-blur-md sticky top-[34px] z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img src="/favicon.png" alt="S" className="w-8 h-8 rounded-full shadow-[0_0_15px_rgba(220,38,38,0.5)]" onError={(e) => e.currentTarget.style.display='none'} />
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-8">
            ...
            <div key={post.id} className="space-y-8">
              <article className="bg-[#161920] border border-white/5 group hover:border-red-600/30 transition-all shadow-2xl rounded-sm overflow-hidden flex flex-col md:flex-row">
                <div className="md:w-2/5 relative overflow-hidden cursor-pointer min-h-[220px]" onClick={() => setSelectedPost(post)}>
                  ...
                  <div className="absolute top-4 left-4 bg-red-600 px-3 py-1 text-[10px] font-black uppercase shadow-lg">ЭКСКЛЮЗИВ</div>
                </div>
              ...
              {(index + 1) % 3 === 0 && (
                <a href={WINLINE_URL} target="_blank" rel="noopener noreferrer" className="block w-full border-2 border-[#ff5c00] hover:opacity-90 transition-all overflow-hidden rounded-sm shadow-[0_0_30px_rgba(255,92,0,0.15)]">
                  <img 
                    src={index % 2 === 0 ? BANNER_1 : BANNER_2} 
                    alt="Winline Promo" 
        ...
        <aside className="w-full lg:w-80 space-y-6">
          <div className="sticky top-32 space-y-6">
            <a href={WINLINE_URL} target="_blank" rel="noopener noreferrer" className="block bg-[#161920] p-1 border-2 border-[#ff5c00] hover:scale-[1.03] transition-transform shadow-[0_0_20px_rgba(255,92,0,0.2)]">
              <div className="bg-[#ff5c00] p-6 text-center">
                ...
                <span className="block text-white font-bold text-[10px] uppercase mt-2 bg-black py-1 tracking-widest">ФРИБЕТ 3000₽</span>
              </div>
            </a>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/98 backdrop-blur-xl overflow-y-auto">
          <div className="relative bg-[#161920] max-w-5xl w-full my-auto border border-white/10 shadow-2xl flex flex-col md:flex-row rounded-sm overflow-hidden">
            ...
            <div className="md:w-2/3 p-6 md:p-12 border-r border-white/5 bg-[#1a1d24]">
              ...
              <a href={WINLINE_URL} target="_blank" rel="noopener noreferrer" className="block mb-10 border-2 border-[#ff5c00] hover:scale-[1.01] transition-transform overflow-hidden rounded-sm">
                 <img src={BANNER_2} alt="Winline" className="w-full h-auto" />
              </a>
              ...
              <a href={WINLINE_URL} target="_blank" rel="noopener noreferrer" className="block w-full bg-[#ff5c00] text-black text-center py-6 font-black uppercase italic text-3xl hover:bg-white transition-all shadow-[0_0_50px_rgba(255,92,0,0.4)]">
                ЗАБРАТЬ 3000₽ И ПОСТАВИТЬ 🚀
              </a>
            </div>

            <div className="md:w-1/3 p-8 bg-black/40 hidden md:block">
              <h3 className="font-black text-[10px] uppercase tracking-widest mb-8 text-red-500 underline decoration-2 underline-offset-8 italic">ЕЩЕ БОЛЬШЕ ИНСАЙДОВ:</h3>
              <div className="space-y-8">
                {posts.filter(p => p.id !== selectedPost.id).slice(0, 4).map(p => (
                  <div key={p.id} onClick={() => setSelectedPost(p)} className="cursor-pointer group border-b border-white/5 pb-6 last:border-0">
                    <div className="relative overflow-hidden mb-3 h-28 border border-white/5">
                       <img src={p.image_url} alt="" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all scale-110 group-hover:scale-100" />
                    </div>
