...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0d0f12] text-white font-sans selection:bg-red-600">
      <a href={WINLINE_URL} target="_blank" rel="noopener noreferrer" className="block bg-[#ff5c00] text-black text-center py-2 font-black text-[10px] uppercase tracking-[0.3em] sticky top-0 z-[100]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-8">
        <div className="flex-1 space-y-8">
          {posts.map((post) => (
            <article key={post.id} className="bg-[#161920] border border-white/5 group hover:border-red-600/30 transition-all shadow-xl rounded-sm overflow-hidden flex flex-col md:flex-row">
              <div className="md:w-2/5 relative overflow-hidden cursor-pointer min-h-[200px]" onClick={() => setSelectedPost(post)}>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md overflow-y-auto">
          <div className="relative bg-[#161920] max-w-5xl w-full my-auto border border-white/10 shadow-2xl flex flex-col md:flex-row rounded-sm overflow-hidden">
            ...
            <div className="md:w-1/3 p-8 bg-black/40 hidden md:block">
              <h3 className="font-black text-[10px] uppercase tracking-widest mb-8 text-red-500 underline decoration-2 underline-offset-8">ЧИТАЙТЕ ТАКЖЕ:</h3>
              <div className="space-y-8">
                {posts.filter(p => p.id !== selectedPost.id).slice(0, 3).map(p => (
                  <div key={p.id} onClick={() => setSelectedPost(p)} className="cursor-pointer group border-b border-white/5 pb-6 last:border-0">
                    <img src={p.image_url} alt="" className="w-full h-28 object-cover mb-4 grayscale group-hover:grayscale-0 transition-all border border-white/5" />
